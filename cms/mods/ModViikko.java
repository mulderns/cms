...
"ja voi olla pidempi ja yksityis kohtaisempi.");
					...
					public void execute(){
						if(ext != ""){
							...
							if((res = db.removeTemlate(ext)) == null){
								...
								box.addParagraph("poisto ep‰onnistui id["+ext+"]:"+res);
		...
		actions.add(new Action(null, "pohjat"){public void execute(){
			if(!ext.trim().equals("")){
				...
				CmsBoxi box = new CmsBoxi("Lis‰‰ tapahtuma");
				if(ve != null){
					{
					...
					box.addP("pohjaa ei lˆytynyt id["+ext+"]");
			...
			if(ext != ""){
				if(checkFields(forma.getFields())){
					...
					if(ve != null){
						...
						CmsBoxi box = new CmsBoxi("Muokkaus sivu");
						box.addP("tapahtumaa ei lˆytynyt id["+ext+"]");
						page.addCenter(box.toString());
					}				
			...
			if(!ext.equals("")){
				if(ext.length() > 0 && ext.length() < 3){
					try{
						...
						boxb.addTag("<p style=\"font-size:9.5px;\">");
						boxb.addTag(getViikkoMail(offset,true));
					...
					if(ve != null){
						...
						esikatselu.addP("Virhe tapahtumaa["+ext+"] ei saatu ladattua");
						page.setTitle("esikatselu");
						page.addTop(getMenu());
						page.addLeft(getActionLinks().toString());
			...
			if(ext == ""){
				...
				if(checkField("saaja")){
					...
					if(result == null){
						...
						boxi.addP("Posti l‰hetettiin onnistuneesti ["+datarelay.post.get("saaja")+"]");
